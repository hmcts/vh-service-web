...
namespace ServiceWebsite.AcceptanceTests.Helpers
{
    public static class NgDriverExtension
    {
        ...
        public static IWebElement WaitUntilElementClickable(this IWebDriver driver, IWebElement element, int timeout = 20)
        {
            ...
            {
                var wait = new WebDriverWait(driver, TimeSpan.FromSeconds(timeout));
                return wait.Until(OpenQA.Selenium.Support.UI.ExpectedConditions.ElementToBeClickable(element));
            }
            catch (NoSuchElementException ex)
            {
        ...
        public static IList<IWebElement> WaitUntilElementsVisible(this IWebDriver driver, By elementLocator, int timeout = 20)
        {
            try
            {
                var wait = new WebDriverWait(driver, TimeSpan.FromSeconds(timeout));
