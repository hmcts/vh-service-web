...
import {
  ParticipantSuitabilityModel,
  MediaAccessResponse,
} from "../../../base-journey/participant-suitability.model";
...
export class TestYourEquipmentComponent
  extends SuitabilityChoicePageBaseComponent<JourneyBase>
  ...
  setupSubscribers() {
    this.$subcriptions.push(
      this.userMediaService.connectedDevices.subscribe(async () => {
        ...
        this.logger.event(
          `(setupSubscribers) Has multiple devices: ${this.hasMultipleDevices}`,
          {
  ...
  async getToken() {
    ...
    const response = await this.videoWebService
      .getCurrentParticipantId()
      .toPromise<ParticipantResponse>();
    this.participantId = response.id;
    try {
      const token = await this.videoWebService
        .getToken(this.participantId)
  ...
  call() {
    ...
    const tokenOptions = btoa(
      `${this.token.expires_on};${this.participantId};${this.token.token}`
    );
    if (this.pexipAPI) {
  ...
  get streamsActive() {
    return (
      this.outgoingStream &&
      this.outgoingStream.active &&
      this.incomingStream &&
      this.incomingStream.active
    );
  }
  ...
  async updatePexipAudioVideoSource() {
    ...
    if (cam) {
      ...
      this.logger.event(
        "(updatePexipAudioVideoSource -> Assigning Video Source.)",
        {
          hearingId: this.model.hearing.id,
          participantId: this.model.participantId,
        }
    ...
    if (mic) {
      ...
      this.logger.event(
        "(updatePexipAudioVideoSource -> Assigning Audio Source.)",
        {
          hearingId: this.model.hearing.id,
          participantId: this.model.participantId,
        }
    ...
    this.preferredMicrophoneStream = await this.userMediaStreamService.getStreamForMic(
      mic
    );
  }
  ...
  errorHandleEvent(reason) {
    this.displayFeed = false;
    this.logger.error(
      "(setupPexipClient -> pexipAPI.onError)",
      new Error(reason),
      {
        hearingId: this.model.hearing.id,
        participantId: this.model.participantId,
    ...
    if (
      reason.toString().includes("Could not get access to camera/microphone")
    ) {
      this.showEquipmentBlockedMessage();
    }
  }
  ...
  disconnectHandleEvent(reason) {
    ...
    this.logger.error(
      "(disconnectHandleEvent -> Disconnected from pexip.)",
      new Error(reason),
      {
        hearingId: this.model.hearing.id,
        participantId: this.model.participantId,
  ...
  retrieveSelfTestScore() {
    ...
    this.$subcriptions.push(
      this.videoWebService.getTestCallScore(this.participantId).subscribe(
        (score) => {
          this.logger.event(
            `(retrieveSelfTestScore -> TEST SCORE KINLY RESULT: ${score.score})`,
            {
              hearingId: this.model.hearing.id,
              participantId: this.model.participantId,
            }
          ...
          this.logger.event(
            `telemetry:serviceweb:any:selftest:score:${score.score}`
          );
        ...
        (error) => {
          ...
          this.logger.error(
            "(retrieveSelfTestScore -> Error to get self test score)",
            new Error(error),
            {
              hearingId: this.model.hearing.id,
              participantId: this.model.participantId,
  ...
  async replayVideo() {
    ...
    if (
      !this.mediaSwitchedOn.result &&
      this.mediaSwitchedOn.exceptionType === this.NotAllowedError
    ) {
      this.showEquipmentBlockedMessage();
    } else {
      this.call();
  ...
  async onMediaDeviceChangeAccepted(
    selectedMediaDevice: SelectedUserMediaDevice
  ) {
    this.displayDeviceChangeModal = false;
    this.userMediaService.updatePreferredCamera(
      selectedMediaDevice.selectedCamera
    );
    this.userMediaService.updatePreferredMicrophone(
      selectedMediaDevice.selectedMicrophone
    );
    await this.updatePexipAudioVideoSource();
    this.call();
  }
